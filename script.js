...
function showPage(n){
   ...
   i=n-1;
    ...
    outer=document.querySelector(".outerContainer")
     ...
     outer.innerHTML=``;
for(i=j;i<k;i++){
    outer.innerHTML +=`<div class=innerContainer><div class=image><img src='https://thumbs.dreamstime.com/b/businessman-icon-image-male-avatar-profile-vector-glasses-beard-hairstyle-179728610.jpg'></div>
  
    <div ><p class=number></p>${a[i].id})</div>

    <div ><h2 class=name></h2>${a[i].name}</div>
   <div ><p class=email></p>${a[i].email}</div>
 </div>`    
}
